...
Push-Location -Path crates/mav
$channel = Get-Content "RELEASE_CHANNEL"
$env:MAV_RELEASE_CHANNEL = $channel
...
function CheckEnvironmentVariables {
    ...
    $requiredVars = @(
        'MAV_WORKSPACE', 'RELEASE_VERSION', 'MAV_RELEASE_CHANNEL',
...
function PrepareForBundle {
    ...
    Copy-Item -Path "$env:MAV_WORKSPACE\crates\mav\resources\windows\*" -Destination "$innoDir" -Recurse -Force
...
function BuildMavAndItsFriends {
    Write-Output "Building Mav and its friends, for channel: $channel"
    # Build mav.exe, cli.exe and auto_update_helper.exe
    cargo build --release --package mav --package cli --package auto_update_helper --target $target
    Copy-Item -Path ".\$CargoOutDir\mav.exe" -Destination "$innoDir\Mav.exe" -Force
    ...
    Copy-Item -Path ".\$CargoOutDir\explorer_command_injector.dll" -Destination "$innoDir\mav_explorer_command_injector.dll" -Force
}
...
function BuildRemoteServer {
    ...
    $remoteServerDst = "$env:MAV_WORKSPACE\target\mav-remote-server-windows-$Architecture.zip"
...
function ZipMavAndItsFriendsDebug {
    $items = @(
        ".\$CargoOutDir\mav.pdb",
        ".\$CargoOutDir\cli.pdb",
    ...
    Compress-Archive -Path $items -DestinationPath ".\$CargoOutDir\mav-$env:RELEASE_VERSION-$env:MAV_RELEASE_CHANNEL.dbg.zip" -Force
}
...
function UploadToSentry {
    ...
    Write-Output "Uploading mav debug symbols to sentry..."
    for ($i = 1; $i -le 3; $i++) {
        try {
            sentry-cli debug-files upload --include-sources --wait -p mav -o mav-dev $CargoOutDir
            break
        }
        catch {
...
function MakeAppx {
    switch ($channel) {
        "stable" {
            $manifestFile = "$env:MAV_WORKSPACE\crates\explorer_command_injector\AppxManifest.xml"
        }
        "preview" {
            $manifestFile = "$env:MAV_WORKSPACE\crates\explorer_command_injector\AppxManifest-Preview.xml"
        }
        default {
            $manifestFile = "$env:MAV_WORKSPACE\crates\explorer_command_injector\AppxManifest-Nightly.xml"
        }
    }
    ...
    $env:Path += ';' + $sdk
    makeAppx.exe pack /d "$innoDir\make_appx" /p "$innoDir\mav_explorer_command_injector.appx" /nv
}

function SignMavAndItsFriends {
    if (-not $env:CI) {
        return
    }

    $files = "$innoDir\Mav.exe,$innoDir\cli.exe,$innoDir\auto_update_helper.exe,$innoDir\mav_explorer_command_injector.dll,$innoDir\mav_explorer_command_injector.appx"
...
function CollectFiles {
    Move-Item -Path "$innoDir\mav_explorer_command_injector.appx" -Destination "$innoDir\appx\mav_explorer_command_injector.appx" -Force
    Move-Item -Path "$innoDir\mav_explorer_command_injector.dll" -Destination "$innoDir\appx\mav_explorer_command_injector.dll" -Force
    Move-Item -Path "$innoDir\cli.exe" -Destination "$innoDir\bin\mav.exe" -Force
    Move-Item -Path "$innoDir\mav.sh" -Destination "$innoDir\bin\mav" -Force
...
function BuildInstaller {
    ...
    switch ($channel) {
        "stable" {
            ...
            $appName = "Mav"
            $appDisplayName = "Mav"
            $appSetupName = "Mav-$Architecture"
            # The mutex name here should match the mutex name in crates\mav\src\mav\windows_only_instance.rs
            $appMutex = "Mav-Stable-Instance-Mutex"
            $appExeName = "Mav"
            $regValueName = "Mav"
            $appUserId = "MavIndustries.Mav"
            $appShellNameShort = "Z&ed"
            $appAppxFullName = "MavIndustries.Mav_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "preview" {
            $appId = "{{F70E4811-D0E2-4D88-AC99-D63752799F95}"
            $appIconName = "app-icon-preview"
            $appName = "Mav Preview"
            $appDisplayName = "Mav Preview"
            $appSetupName = "Mav-$Architecture"
            # The mutex name here should match the mutex name in crates\mav\src\mav\windows_only_instance.rs
            $appMutex = "Mav-Preview-Instance-Mutex"
            $appExeName = "Mav"
            $regValueName = "MavPreview"
            $appUserId = "MavIndustries.Mav.Preview"
            $appShellNameShort = "Z&ed Preview"
            $appAppxFullName = "MavIndustries.Mav.Preview_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "nightly" {
            $appId = "{{1BDB21D3-14E7-433C-843C-9C97382B2FE0}"
            $appIconName = "app-icon-nightly"
            $appName = "Mav Nightly"
            $appDisplayName = "Mav Nightly"
            $appSetupName = "Mav-$Architecture"
            # The mutex name here should match the mutex name in crates\mav\src\mav\windows_only_instance.rs
            $appMutex = "Mav-Nightly-Instance-Mutex"
            $appExeName = "Mav"
            $regValueName = "MavNightly"
            $appUserId = "MavIndustries.Mav.Nightly"
            $appShellNameShort = "Z&ed Editor Nightly"
            $appAppxFullName = "MavIndustries.Mav.Nightly_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "dev" {
            $appId = "{{8357632E-24A4-4F32-BA97-E575B4D1FE5D}"
            $appIconName = "app-icon-dev"
            $appName = "Mav Dev"
            $appDisplayName = "Mav Dev"
            $appSetupName = "Mav-$Architecture"
            # The mutex name here should match the mutex name in crates\mav\src\mav\windows_only_instance.rs
            $appMutex = "Mav-Dev-Instance-Mutex"
            $appExeName = "Mav"
            $regValueName = "MavDev"
            $appUserId = "MavIndustries.Mav.Dev"
            $appShellNameShort = "Z&ed Dev"
            $appAppxFullName = "MavIndustries.Mav.Dev_1.0.0.0_neutral__japxn1gcva8rg"
        }
        default {
    ...
    $definitions = @{
        ...
        "OutputDir"      = "$env:MAV_WORKSPACE\target"
...
ParseMavWorkspace
$innoDir = "$env:MAV_WORKSPACE\inno\$Architecture"
$debugArchive = "$CargoOutDir\mav-$env:RELEASE_VERSION-$env:MAV_RELEASE_CHANNEL.dbg.zip"
$debugStoreKey = "$env:MAV_RELEASE_CHANNEL/mav-$env:RELEASE_VERSION-$env:MAV_RELEASE_CHANNEL.dbg.zip"

CheckEnvironmentVariables
PrepareForBundle
GenerateLicenses
BuildMavAndItsFriends
BuildRemoteServer
MakeAppx
SignMavAndItsFriends
ZipMavAndItsFriendsDebug
...
if ($buildSuccess) {
    ...
    if ($Install) {
        Write-Output "Installing Mav..."
        Start-Process -FilePath "$env:MAV_WORKSPACE/target/MavEditorUserSetup-x64-$env:RELEASE_VERSION.exe"
    }
    exit 0
}
else {
