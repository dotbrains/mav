mod go_locator {
    ...
    async fn test_create_scenario_for_go_build(_: &mut TestAppContext) {
        ...
        let task = TaskTemplate {
            ...
            env: Default::default(),
            cwd: Some("${MAV_WORKTREE_ROOT}".into()),
...
mod python_locator {
    ...
    async fn test_python_locator() {
        ...
        let build_task = TaskTemplate {
            label: "run module '$MAV_FILE'".into(),
            command: "$MAV_CUSTOM_PYTHON_ACTIVE_MAV_TOOLCHAIN".into(),
            args: vec!["-m".into(), "$MAV_CUSTOM_PYTHON_MODULE_NAME".into()],
            env: Default::default(),
            cwd: Some("$MAV_WORKTREE_ROOT".into()),
            use_new_terminal: false,
            allow_concurrent_runs: false,
        ...
        let expected_scenario = DebugScenario {
            ...
            config: json!({
                "request": "launch",
                "python": "$MAV_CUSTOM_PYTHON_ACTIVE_MAV_TOOLCHAIN",
                "args": [],
                "cwd": "$MAV_WORKTREE_ROOT",
                "module": "$MAV_CUSTOM_PYTHON_MODULE_NAME",
            }),
            tcp_connection: None,
        };
