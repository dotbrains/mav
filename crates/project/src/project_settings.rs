...
pub struct NodeBinarySettings {
    ...
    /// The path to the npm binary Mav should use (defaults to `.path/../npm`).
    pub npm_path: Option<String>,
    /// If enabled, Mav will download its own copy of Node.
...
pub struct DiagnosticsSettings {
    ...
    /// Settings for using LSP pull diagnostics mechanism in Mav.
...
/// SettingsObserver observers changes to .mav/{settings, task}.json files in local worktrees
/// (or the equivalent protobuf messages from upstream) and updates local settings
/// and sends notifications downstream.
/// In ssh mode it also monitors ~/.config/mav/{settings, task}.json and sends the content
/// upstream.
                        ...
                        Some(
                            async move {
                                ...
                                if abs_path.ends_with(local_vscode_tasks_file_relative_path().as_std_path()) {
                                    ...
                                    let mav_tasks = TaskTemplates::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode tasks into Mav ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&mav_tasks).with_context(|| {
                                        format!(
                                            "serializing Mav tasks into JSON, file {abs_path:?}"
                                        )
                                    })
                                    ...
                                    let mav_tasks = DebugTaskFile::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode debug tasks into Mav ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&mav_tasks).with_context(|| {
                                        format!(
