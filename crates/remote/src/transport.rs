...
fn parse_platform(output: &str) -> Result<RemotePlatform> {
    ...
    let os = match os {
        ...
        _ => anyhow::bail!(
            "Prebuilt remote servers are not yet available for {os:?}. See https://mav.dev/docs/remote-development"
        ),
    };
        ...
        || arch.starts_with("aarch64")
    {
        ...
        anyhow::bail!(
            "Prebuilt remote servers are not yet available for {arch:?}. See https://mav.dev/docs/remote-development"
        )
    };
...
async fn build_remote_server_from_source(
    ...
    if let Ok(path) = std::env::var("MAV_COPY_REMOTE_SERVER") {
        let path = std::path::PathBuf::from(path);
        if path.exists() {
            return Ok(Some(path));
        } else {
            log::warn!(
                "MAV_COPY_REMOTE_SERVER path does not exist, falling back to MAV_BUILD_REMOTE_SERVER: {}",
                path.display()
            );
        }
    }
    ...
    let build_remote_server =
        std::env::var("MAV_BUILD_REMOTE_SERVER").unwrap_or("nocompress".into());
    ...
    if let "never" = &*build_remote_server {
        ...
        log::warn!("MAV_BUILD_REMOTE_SERVER is disabled, but no server binary exists on the server")
    }
    ...
    if platform.os == RemoteOs::Linux && use_musl {
        ...
        if let Ok(path) = std::env::var("MAV_ZSTD_MUSL_LIB") {
