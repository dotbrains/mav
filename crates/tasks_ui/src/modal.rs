...
pub use mav_actions::{Rerun, Spawn};
        ...
        fs.insert_tree(
            ...
            json!({
                ".mav": {
                    "tasks.json": r#"[
                        {
                            "label": "hello from $MAV_FILE:$MAV_ROW:$MAV_COLUMN",
                            "command": "echo",
                            "args": ["hello", "from", "$MAV_FILE", ":", "$MAV_ROW", ":", "$MAV_COLUMN"]
                        },
                        {
                            "label": "opened now: $MAV_WORKTREE_ROOT",
                            "command": "echo",
                            "args": ["opened", "now:", "$MAV_WORKTREE_ROOT"]
                        }
                    ]"#,
                },
            ...
            language_registry.add(Arc::new(
                ...
                .with_context_provider(Some(Arc::new(
                    ContextProviderWithTasks::new(TaskTemplates(vec![
                        ...
                        TaskTemplate {
                            label: "TypeScript task from file $MAV_FILE".to_string(),
                            command: "npm run build".to_string(),
                            ..TaskTemplate::default()
                        },
                        TaskTemplate {
                            label: "Another task from file $MAV_FILE".to_string(),
