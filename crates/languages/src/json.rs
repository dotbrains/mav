...
use util::{ResultExt, maybe, merge_json_value_into, paths::PathStyle, rel_path::RelPath};

use crate::PackageJsonData;

mod node_version;
pub use node_version::NodeVersionAdapter;
        ...
        cx.spawn(async move |cx| {
            ...
            let task_templates = if is_package_json {
                ...
                package_json
                    ...
                    .chain([TaskTemplate {
                        label: "package script $MAV_CUSTOM_script".to_owned(),
                        command: command.clone(),
                        args: vec![
                            "run".into(),
                ...
                serde_json_lenient::Value::from_str(&contents.text)
                    ...
                    .map(|key| TaskTemplate {
                        ...
                        args: vec!["-d".into(), "$MAV_DIRNAME".into(), key.into()],
                        ..TaskTemplate::default()
                    })
                    .chain([TaskTemplate {
                        label: "composer script $MAV_CUSTOM_script".to_owned(),
                        command: "composer".to_owned(),
                        args: vec![
                            "-d".into(),
                            "$MAV_DIRNAME".into(),
