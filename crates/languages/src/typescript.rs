...
mod tests {
    ...
    async fn test_outline_with_nested_object_methods(cx: &mut TestAppContext) {
        for language in [
            ...
            let text = r#"
            // Reproduction from https://github.com/mav-industries/mav/issues/48711
            const a = {
              p01: '01',
              fn01: () => {},
              fn02() {},
        ...
        pretty_assertions::assert_eq!(
            ...
            [
                (
                    ...
                    Some("$MAV_CUSTOM_TYPESCRIPT_VITEST_PACKAGE_PATH".into()),
                ),
                (
                    "vitest test $MAV_SYMBOL".into(),
                    Some("$MAV_CUSTOM_TYPESCRIPT_VITEST_PACKAGE_PATH".into()),
                ),
                (
                    "mocha file test".into(),
                    Some("$MAV_CUSTOM_TYPESCRIPT_MOCHA_PACKAGE_PATH".into()),
                ),
                (
                    "mocha test $MAV_SYMBOL".into(),
                    Some("$MAV_CUSTOM_TYPESCRIPT_MOCHA_PACKAGE_PATH".into()),
                ),
                (
