...
pub(super) struct TextRendererWrapper(pub(super) IDWriteTextRenderer);
...
impl TextRenderer {
    pub(super) fn new(locale: HSTRING) -> Self {
        TextRenderer { locale }
    }
}
...
impl<'t> ClusterAnalyzer<'t> {
    pub(super) fn new(cluster_map: &'t [u16], glyph_count: usize) -> Self {
        ClusterAnalyzer {
            utf16_idx: 0,
            glyph_idx: 0,
            glyph_count,
