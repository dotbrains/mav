...
impl DirectWriteState {
    ...
    pub(super) fn get_typographic_bounds(
        &self,
        font_id: FontId,
        glyph_id: GlyphId,
    ) -> Result<Bounds<f32>> {
    ...
    pub(super) fn get_advance(&self, font_id: FontId, glyph_id: GlyphId) -> Result<Size<f32>> {
        unsafe {
            let font = &self.fonts[font_id.0].font_face;
            let glyph_indices = [glyph_id.0 as u16];
            let mut metrics = [DWRITE_GLYPH_METRICS::default()];
    ...
    pub(super) fn all_font_names(&self, components: &DirectWriteComponents) -> Vec<String> {
        let mut result =
    ...
    pub(super) fn handle_gpu_lost(&mut self, directx_devices: &DirectXDevices) -> Result<()> {
