...
mod tests {
    ...
    fn test_variable_substitution() {
        ...
        assert_eq!(
            replacer.replace("$PATH is an environment variable"),
            "${MAV_PATH} is an environment variable"
        );
        assert_eq!(replacer.replace("${PATH}"), "${MAV_PATH}");
        assert_eq!(replacer.replace("${PATH:food}"), "${MAV_PATH:food}");
    }
    ...
    fn can_deserialize_ts_tasks() {
        ...
        let expected = vec![
            ...
            TaskTemplate {
                ...
                command: "node".to_string(),
                args: vec![
                    "${MAV_WORKTREE_ROOT}/node_modules/typescript/lib/tsc.js".to_string(),
                    "--build".to_string(),
                    "${MAV_WORKTREE_ROOT}/src".to_string(),
                    "--watch".to_string(),
                ],
                ..Default::default()
            },
