...
impl UserStore {
    ...
    pub fn plan(&self) -> Option<Plan> {
        ...
        if let Ok(plan) = std::env::var("MAV_SIMULATE_PLAN").as_ref() {
            use cloud_api_client::Plan;

            return match plan.as_str() {
                "free" => Some(Plan::MavFree),
                "trial" => Some(Plan::MavProTrial),
                "pro" => Some(Plan::MavPro),
                _ => {
                    panic!("MAV_SIMULATE_PLAN must be one of 'free', 'trial', or 'pro'");
                }
            };
        }
    ...
    fn update_authenticated_user(
        ...
        let staff = response.user.is_staff && !*feature_flags::MAV_DISABLE_STAFF;
