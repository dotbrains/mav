...
pub fn init(app_state: Arc<AppState>, cx: &mut App) {
    cx.on_action(|_: &mav_actions::dev::ToggleInspector, cx| {
    ...
    v_flex()
        ...
        .child(
            div()
                ...
                .tooltip(Tooltip::text("Click to open by running Mav CLI"))
                .on_click(move |_, _window, cx| {
                    cx.background_spawn(open_mav_source_location(source_location))
...
async fn open_mav_source_location(
    location: &'static std::panic::Location<'static>,
) -> anyhow::Result<()> {
    let mut path = Path::new(env!("MAV_REPO_DIR")).to_path_buf();
