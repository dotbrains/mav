...
pub enum MentionUri {
    ...
    /// verbatim so re-saved threads stay loadable by older Mav versions.
    Rule {
    ...
    pub fn to_uri(&self) -> Url {
        match self {
            ...
            MentionUri::PastedImage { name } => {
                let mut url = Url::parse("mav:///agent/pasted-image").unwrap();
            ...
            MentionUri::Selection {
                ...
                let mut url = if let Some(path) = abs_path {
                    ...
                    let mut url = Url::parse("mav:///").unwrap();
            ...
            MentionUri::TerminalSelection { line_count } => {
                let mut url = Url::parse("mav:///agent/terminal-selection").unwrap();
                url.query_pairs_mut()
                    .append_pair("lines", &line_count.to_string());
                url
            }
            MentionUri::GitDiff { base_ref } => {
                let mut url = Url::parse("mav:///agent/git-diff").unwrap();
                url.query_pairs_mut().append_pair("base", base_ref);
                url
            }
            MentionUri::MergeConflict { file_path } => {
                let mut url = Url::parse("mav:///agent/merge-conflict").unwrap();
...
/// Placeholder rule `id` for legacy mentions missing one, shaped so older Mav
...
mod tests {
    ...
    fn test_parse_untitled_selection_uri() {
        let selection_uri = uri!("mav:///agent/untitled-buffer#L1:10");
        let parsed = MentionUri::parse(selection_uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: None,
    ...
    fn test_parse_thread_uri() {
        let thread_uri = "mav:///agent/thread/session123?name=Thread+name";
    ...
    fn test_parse_legacy_rule_uri() {
        let rule_uri = "mav:///agent/rule/d8694ff2-90d5-4b6f-be33-33c1763acd52?name=Some+rule";
    ...
    fn test_legacy_rule_mention_preserves_id() {
        // The `id` older Mav versions require must survive a load + save.
    ...
    fn test_parse_diagnostics_uri() {
        let uri = "mav:///agent/diagnostics?include_warnings=true";
    ...
    fn test_parse_diagnostics_uri_warnings_only() {
        let uri = "mav:///agent/diagnostics?include_warnings=true&include_errors=false";
        let parsed = MentionUri::parse(uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Diagnostics {
                include_errors,
    ...
    #[test]
    fn test_invalid_mav_path() {
        assert!(MentionUri::parse("mav:///invalid/path", PathStyle::local()).is_err());
        assert!(MentionUri::parse("mav:///agent/unknown/test", PathStyle::local()).is_err());
    }
    ...
    fn test_parse_absolute_windows_path() {
        let file_path = "C:\\Users\\mav\\project\\main.rs";
        let parsed = MentionUri::parse(file_path, PathStyle::Windows).unwrap();
        match &parsed {
            MentionUri::File { abs_path } => {
                assert_eq!(abs_path, Path::new("C:\\Users\\mav\\project\\main.rs"));
            }
            _ => panic!("Expected File variant"),
        }
    }
    ...
    fn test_parse_absolute_windows_file_path_with_row() {
        let file_path = "C:\\Users\\mav\\project\\main.rs:42";
        let parsed = MentionUri::parse(file_path, PathStyle::Windows).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: path,
                ...
                assert_eq!(
                    path.as_ref().unwrap(),
                    Path::new("C:\\Users\\mav\\project\\main.rs")
                );
                assert_eq!(line_range.start(), &41);
                assert_eq!(line_range.end(), &41);
            }
    ...
    fn test_parse_absolute_windows_file_path_with_fragment_line() {
        let file_path = "C:\\Users\\mav\\project\\main.rs#L42";
        let parsed = MentionUri::parse(file_path, PathStyle::Windows).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: path,
    ...
    fn test_single_line_number() {
        // https://github.com/mav-industries/mav/issues/46114
    ...
    fn test_parse_terminal_selection_uri() {
        let terminal_uri = "mav:///agent/terminal-selection?lines=42";
        ...
        // Test single line
        let single_line_uri = "mav:///agent/terminal-selection?lines=1";
