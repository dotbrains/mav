...
///
/// By default, running Mav in development uses the development credentials
...
static MAV_DEVELOPMENT_USE_KEYCHAIN: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("MAV_DEVELOPMENT_USE_KEYCHAIN").is_ok_and(|value| !value.is_empty())
});

pub struct MavCredentialsProvider(pub Arc<dyn CredentialsProvider>);

impl Global for MavCredentialsProvider {}
...
pub fn init_global(cx: &mut App) {
    ...
    let provider = new(cx);
    cx.set_global(MavCredentialsProvider(provider));
}

pub fn global(cx: &App) -> Arc<dyn CredentialsProvider> {
    cx.try_global::<MavCredentialsProvider>()
...
fn new(cx: &App) -> Arc<dyn CredentialsProvider> {
    let use_development_provider = match ReleaseChannel::try_global(cx) {
        Some(ReleaseChannel::Dev) => {
            ...
            //
            // However, if the `MAV_DEVELOPMENT_USE_KEYCHAIN` environment
            // variable is set, we will use the actual keychain.
            !*MAV_DEVELOPMENT_USE_KEYCHAIN
        }
...
/// re-allow access to the system keychain when developing Mav.
