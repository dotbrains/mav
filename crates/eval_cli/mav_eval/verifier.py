...
class MavJudgeProxyVerifier(Verifier):
    ...
    async def _ensure_judge_proxy(self) -> None:
        ...
        if not merged_env.get("MAV_JUDGE_UPSTREAM"):
            return
        ...
        result = await self.environment.exec(
            command=(
                ...
                "cat > /usr/local/lib/mav_judge_proxy.py <<'MAV_PROXY_EOF'\n"
                f"{JUDGE_PROXY_SCRIPT}"
                "MAV_PROXY_EOF\n"
                "cat > /usr/local/bin/ensure-judge-proxy <<'MAV_ENSURE_EOF'\n"
                f"{JUDGE_PROXY_ENSURE_SCRIPT}"
                "MAV_ENSURE_EOF\n"
                "chmod +x /usr/local/bin/ensure-judge-proxy\n"
                "ensure-judge-proxy\n"
                "python3 - <<'MAV_PROXY_CHECK_EOF'\n"
                "import os, socket\n"
