...
from mav_eval import benchmarks, config, harness_command
...
class HarnessCommandTests(unittest.TestCase):
    ...
    def test_swe_atlas_rf_wires_judge(self) -> None:
        ...
        self.assertTrue(
            any(arg.startswith("MAV_JUDGE_UPSTREAM=") for arg in verifier_env)
        )
        self.assertTrue(
            any(arg.startswith("MAV_JUDGE_AUTH_ENV=") for arg in verifier_env)
        )
        self.assertIn("MAV_JUDGE_MAX_TOKENS=8192", verifier_env)
        self.assertFalse(any(arg.startswith("MAV_JUDGE_") for arg in agent_env))
    ...
    def test_deepswe_uses_pier_path_without_cli_allowlist(self) -> None:
        ...
        self.assertIn("mav_eval.pier_agent:MavPierAgent", command)
    ...
    def test_harbor_benchmarks_use_harbor_agent(self) -> None:
        ...
        self.assertIn("mav_eval.agent:MavAgent", command)
    ...
    def test_extra_env_is_forwarded_to_harness_agent(self) -> None:
        command = harness_command.build_harness_command(
            make_run_request(
                "swe-atlas-rf",
                extra_env={
                    "MAV_EVAL_INSTRUCTION_SUFFIX_FILE": "/data/prompts/checklist.md"
                },
            ),
            "/tmp/jobs",
        )
