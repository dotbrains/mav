"""Pier-native agent wrapper for Mav's eval-cli binary.
...
class MavPierAgent(BaseInstalledAgent):
    """Runs Mav's headless eval-cli binary under Pier."""
    ...
    def _container_binary_path(self) -> str:
        ...
        if not path:
            raise ValueError(
                "MavPierAgent requires EVAL_CLI_CONTAINER_PATH (the eval-cli binary "
    ...
    def network_allowlist(self) -> NetworkAllowlist:
        ...
        providers_json = self._get_env("MAV_OPENAI_COMPATIBLE_PROVIDERS")
        if providers_json:
            try:
    ...
    def _api_env(self) -> dict[str, str]:
        ...
        add_openai_compatible_provider_env(
            env, self._get_env("MAV_OPENAI_COMPATIBLE_PROVIDERS")
        )
        add_anthropic_available_models_env(
            env, self._get_env("MAV_ANTHROPIC_AVAILABLE_MODELS")
        )
        add_mav_eval_env(
    ...
    async def run(
        ...
        instruction_suffix_file = self._get_env("MAV_EVAL_INSTRUCTION_SUFFIX_FILE")
