...
The per-trial record has two sources, one canonical each, with no fallbacks:
  ...
  - Resource usage (tokens / steps / tool calls) comes from the Mav agent's
...
MAV_RESULT_KEYS = {"status", "duration_secs"}
...
def load_mav_result(trial_dir: Path) -> dict | None:
    for path in sorted(trial_dir.rglob("result.json")):
        if path.parent == trial_dir:
            continue
        data = load_json(path)
        if isinstance(data, dict) and MAV_RESULT_KEYS & data.keys():
            return data
    return None
...
def extract_trial(
    ...
    mav_result = load_mav_result(trial_dir)
    if mav_result:
        record.has_result = True
        record.status = mav_result.get("status")
        record.model = mav_result.get("model")
        record.input_tokens = int(mav_result.get("input_tokens") or 0)
        record.output_tokens = int(mav_result.get("output_tokens") or 0)
        record.cache_read_input_tokens = int(
            mav_result.get("cache_read_input_tokens") or 0
        )
        record.cache_creation_input_tokens = int(
            mav_result.get("cache_creation_input_tokens") or 0
        )
        duration = mav_result.get("duration_secs")
        if isinstance(duration, (int, float)):
            record.duration_secs = float(duration)
        # The canonical metrics source: counts eval-cli writes into result.json.
        if isinstance(mav_result.get("step_count"), int):
            record.step_count = mav_result["step_count"]
        if isinstance(mav_result.get("tool_call_count"), int):
            record.total_tool_calls = mav_result["tool_call_count"]
        if isinstance(mav_result.get("tool_calls"), dict):
            record.tool_calls = {
                str(name): int(count)
                for name, count in mav_result["tool_calls"].items()
