...
def add_openai_compatible_provider_env(
    ...
    if providers_json:
        env["MAV_OPENAI_COMPATIBLE_PROVIDERS"] = providers_json
...
def add_anthropic_available_models_env(
    ...
    if models_json:
        env["MAV_ANTHROPIC_AVAILABLE_MODELS"] = models_json


def add_mav_eval_env(
    env: dict[str, str], extra_env: dict[str, str], *, exclude: set[str] | None = None
) -> None:
    exclude = exclude or set()
    for key, value in extra_env.items():
        if key.startswith("MAV_EVAL_") and key not in exclude:
            env[key] = value
