...
def build_harness_command(run_request: dict[str, Any], jobs_dir: str) -> list[str]:
    ...
    command = [
        ...
        *dataset_args(benchmark),
        ...
        "--agent-import-path",
        (
            "mav_eval.pier_agent:MavPierAgent"
            if harness_binary(benchmark) == benchmarks.HARNESS_PIER
            else "mav_eval.agent:MavAgent"
        ),
        "--ae",
    ...
    if openai_compatible_provider_json:
        command.extend(
            [
                "--ae",
                f"MAV_OPENAI_COMPATIBLE_PROVIDERS={openai_compatible_provider_json}",
            ]
        )
    ...
    if anthropic_available_models_json:
        command.extend(
            [
                "--ae",
                f"MAV_ANTHROPIC_AVAILABLE_MODELS={anthropic_available_models_json}",
            ]
        )
