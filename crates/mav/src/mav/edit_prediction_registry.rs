...
use edit_prediction::{EditPredictionModel, MavEditPredictionDelegate};
use editor::Editor;
...
fn edit_prediction_provider_config_for_settings(cx: &App) -> Option<EditPredictionProviderConfig> {
    ...
    match provider {
        ...
        EditPredictionProvider::Mav => {
            Some(EditPredictionProviderConfig::Mav(EditPredictionModel::Zeta))
        }
        ...
        EditPredictionProvider::Ollama | EditPredictionProvider::OpenAiCompatibleApi => {
            ...
            if matches!(format, EditPredictionPromptFormat::Zeta(_)) {
                ...
                Some(EditPredictionProviderConfig::Mav(
                    EditPredictionModel::Fim { format },
                ))
            }
        }

        EditPredictionProvider::Mercury => Some(EditPredictionProviderConfig::Mav(
...
impl EditPredictionProviderConfig {
    fn name(&self) -> &'static str {
        match self {
            ...
            EditPredictionProviderConfig::Mav(model) => match model {
...
fn assign_edit_prediction_provider(
    ...
    match provider_config {
        None => {
            editor.set_edit_prediction_provider::<MavEditPredictionDelegate>(None, window, cx);
        }
        ...
        Some(EditPredictionProviderConfig::Mav(model)) => {
            let ep_store = edit_prediction::EditPredictionStore::global(client, &user_store, cx);
                ...
                user_store.read(cx).current_organization_configuration()
            {
                if !organization_configuration.edit_prediction.is_enabled {
                    editor.set_edit_prediction_provider::<MavEditPredictionDelegate>(
                        None, window, cx,
                    );
...
mod tests {
    ...
    async fn test_subscribe_uses_stale_provider_config_after_settings_change(
        ...
        // captures None at init time. (The test default is Mav/Zeta1, which
