...
impl MoveToApplicationsRequest {
    ...
    async fn prompt(
        ...
        let response = cx
            .prompt(
                PromptLevel::Info,
                "Move Mav to Applications?",
                Some(
                    "Mav is running from a temporary location. Move it to Applications to finish installing it.",
                ),
                &[
                    PromptButton::ok("Yes"),
            ...
            0 => {
                workspace
                    .update_in(cx, |workspace, window, cx| {
                        workspace
                            .toggle_modal(window, cx, |_window, cx| InstallingMavModal::new(cx));
                    })
                    .ok();
                if let Err(error) = move_to_applications(&self.app_path, cx).await {
                    workspace
                        .update_in(cx, |workspace, _window, cx| {
                            if let Some(modal) = workspace.active_modal::<InstallingMavModal>(cx) {
                    ...
                    cx.prompt(
                        ...
                        "Failed to move Mav to Applications",
...
pub struct InstallingMavModal {
    focus_handle: FocusHandle,
    finished: bool,
}

impl InstallingMavModal {
...
impl EventEmitter<DismissEvent> for InstallingMavModal {}

impl ModalView for InstallingMavModal {
...
impl Focusable for InstallingMavModal {
    fn focus_handle(&self, _cx: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for InstallingMavModal {
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .child(Label::new("Installing Mav…")),
            )
            .child(
                h_flex()
                    .w_full()
                    ...
                    .child(
                        v_flex()
                            .gap_1()
                            .child(Label::new("Moving Mav to Applications"))
                            .child(
                                Label::new("Mav will reopen when installation is complete.")
