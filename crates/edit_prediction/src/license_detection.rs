...
mod tests {
    ...
    */
    ...
    async fn test_watcher_updates_on_changes(cx: &mut TestAppContext) {
        ...
        fs.write(Path::new("/root/LICENSE"), APACHE_2_0_TXT.as_bytes())
            .await
            .unwrap();
        ...
        // Still considered open source as LICENSE is present
        cx.background_executor.run_until_parked();
        assert!(watcher.is_project_open_source());

        fs.write(Path::new("/root/LICENSE"), "Also nevermind".as_bytes())
            .await
            .unwrap();
