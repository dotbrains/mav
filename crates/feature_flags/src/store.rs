...
use crate::{FeatureFlag, FeatureFlagValue, FeatureFlagsSettings, MAV_DISABLE_STAFF};
...
impl FeatureFlagStore {
    ...
    /// Debug builds are always treated as staff, and `MAV_DISABLE_STAFF`
    /// forces the user to be treated as non-staff for testing.
    pub fn overrides_enabled(&self) -> bool {
        (cfg!(debug_assertions) || self.staff) && !*MAV_DISABLE_STAFF
    }
    ...
    pub fn try_flag_value<T: FeatureFlag>(&self, cx: &App) -> Option<T::Value> {
        ...
        if (cfg!(debug_assertions) || self.staff) && !*MAV_DISABLE_STAFF && T::enabled_for_staff() {
            return Some(T::Value::on_variant());
        }
    ...
    pub fn has_flag_default<T: FeatureFlag>() -> bool {
        ...
        cfg!(debug_assertions) && T::enabled_for_staff() && !*MAV_DISABLE_STAFF
    }
}
