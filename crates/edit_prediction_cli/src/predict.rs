...
pub async fn run_prediction(
    ...
    ep_store.update(&mut cx, |store, _cx| {
        ...
        // MAV_ZETA_MODEL env var is optional.
        if let PredictionProvider::Zeta2(format) = provider {
            if format != ZetaFormat::default() {
                let model_id = std::env::var("MAV_ZETA_MODEL").ok();
                let environment = std::env::var("MAV_ZETA_ENVIRONMENT").ok();
...
pub async fn predict_baseten(
    ...
    let model_id =
        std::env::var("MAV_ZETA_MODEL").context("MAV_ZETA_MODEL environment variable required")?;
