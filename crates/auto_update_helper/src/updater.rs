...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("Mav.exe"), p("old\\Mav.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\Mav.exe"), p("old\\bin\\Mav.exe")),
        ...
        // Copy new files
        Job::move_file(p("install\\Mav.exe"), p("Mav.exe")),
        Job::move_file(p("install\\bin\\Mav.exe"), p("bin\\Mav.exe")),
    ...
    [
        Job {
            apply: Box::new(|_| {
                ...
                if let Ok(config) = std::env::var("MAV_AUTO_UPDATE") {
                    match config.as_str() {
                        "err1" => Err(std::io::Error::other("Simulated error")).context("Anyhow!"),
                        "err2" => Ok(()),
                        _ => panic!("Unknown MAV_AUTO_UPDATE value: {}", config),
                    }
                } else {
                    Ok(())
                }
            ...
            rollback: Box::new(|_| {
                unsafe { std::env::set_var("MAV_AUTO_UPDATE_RB", "rollback1") };
                Ok(())
            }),
        },
        Job::mkdir(p("test1")),
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("Mav.exe"),
        app_dir.join("bin\\Mav.exe"),
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("Mav.exe")).spawn();
    }
...
mod test {
    ...
    fn test_perform_update() {
        ...
        let app_dir = app_dir.path();
        // Simulate a timeout
        unsafe { std::env::set_var("MAV_AUTO_UPDATE", "err1") };
        ...
        let app_dir = app_dir.path();
        // Simulate a timeout
        unsafe { std::env::set_var("MAV_AUTO_UPDATE", "err2") };
        let ret = perform_update(app_dir, None, false);
        assert!(
            ret.is_err_and(|e| e.to_string().as_str() == "Autoupdate failed, rollback successful")
        );
        assert!(std::env::var("MAV_AUTO_UPDATE_RB").is_ok_and(|e| e == "rollback1"));
    }
}
