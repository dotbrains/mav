...
pub struct Config {
    ...
    pub mav_environment: Arc<str>,
    pub mav_cloud_internal_api_key: String,
    pub mav_client_checksum_seed: Option<String>,
}

impl Config {
    pub fn is_development(&self) -> bool {
        self.mav_environment == "development".into()
    }

    /// Returns the base `mav.dev` URL.
    pub fn mav_dot_dev_url(&self) -> &str {
        match self.mav_environment.as_ref() {
            "development" => "http://localhost:3000",
            "staging" => "https://staging.mav.dev",
            _ => "https://mav.dev",
        }
    }

    /// Returns the base Mav Cloud URL.
    pub fn mav_cloud_url(&self) -> &str {
        match self.mav_environment.as_ref() {
            "development" => "http://localhost:8787",
            _ => "https://cloud.mav.dev",
        }
    }
    ...
    pub fn test() -> Self {
        Self {
            ...
            rust_log: None,
            log_json: None,
            mav_environment: "test".into(),
            mav_cloud_internal_api_key: "test-internal-api-key".into(),
            blob_store_url: None,
            blob_store_region: None,
            blob_store_access_key: None,
            blob_store_secret_key: None,
            blob_store_bucket: None,
            mav_client_checksum_seed: None,
            kinesis_region: None,
    ...
    pub async fn new(config: Config, executor: Executor) -> Result<Arc<Self>> {
        ...
        let this = Self {
            ...
            user_service: Arc::new(CloudUserService::new(
                http_client,
                config.mav_cloud_url().to_string(),
                config.mav_cloud_internal_api_key.clone(),
            )),
            config,
        };
        Ok(Arc::new(this))
