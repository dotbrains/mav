...
use language_model::{IconOrSvg, LanguageModelRegistry, MAV_CLOUD_PROVIDER_ID};
...
impl ApiKeysWithProviders {
    ...
    fn compute_configured_providers(cx: &App) -> Vec<(IconOrSvg, SharedString)> {
        LanguageModelRegistry::read_global(cx)
            ...
            .iter()
            .filter(|provider| {
                provider.is_authenticated(cx) && provider.id() != MAV_CLOUD_PROVIDER_ID
            })
            ...
            .child(
                Button::new("configure-providers", "Configure Providers")
                    ...
                    .on_click(move |_, window, cx| {
                        window.dispatch_action(mav_actions::agent::OpenSettings.boxed_clone(), cx);
                    }),
            )
    }
}
