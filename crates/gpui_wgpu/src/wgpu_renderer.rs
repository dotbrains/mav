...
impl WgpuRenderer {
    ...
    fn create_pipelines(
        ...
        // DUAL_SOURCE_BLENDING. We have a crash report (MAV-5G1) where a
        ...
        if dual_source_blending && !device_has_feature {
            log::error!(
                "BUG: dual_source_blending flag is true but device does not \
                 ...
                 https://github.com/mav-industries/mav/issues",
                device.features(),
            );
        }
...
impl RenderingParameters {
    fn new(adapter: &wgpu::Adapter, surface_format: wgpu::TextureFormat) -> Self {
        ...
        let gamma = env::var("MAV_FONTS_GAMMA")
            .ok()
        ...
        let grayscale_enhanced_contrast = env::var("MAV_FONTS_GRAYSCALE_ENHANCED_CONTRAST")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(1.0_f32)
            .max(0.0);

        let subpixel_enhanced_contrast = env::var("MAV_FONTS_SUBPIXEL_ENHANCED_CONTRAST")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(0.5_f32)
            .max(0.0);
